$(document).ready(function(){
	...
	var isFrmTransactionDetailsSubmit = false;
	...
	$('body').delegate('.btn-see-items','click',function(){
		...
		boxSeeItems = modalForm('Item List', itemList, "mediu,");
	...
	$('body').delegate('.btn-print-transaction','click',function(){
		...
		bootbox.confirm({
			...
			callback : function(result){
				if(result){
					generateOutput(id, page,'createTransactions', outputtype, "walk_in");
				}else{
					getLatestTransactionDetails = ajax({getLatestTransactionDetails :1 }, true);
					boxTransactionSummary = modalForm("Transaction Summary", getLatestTransactionDetails);
				}
	...
	$('body').delegate('.txt-discount','change',function(){
	...
	$('body').delegate('.txt-cash-received','change',function(){
