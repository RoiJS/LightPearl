	...
	<td>
		<div class="div-add-item-qty" >
			<?php if($item['row']['noOfItem'] == 0){?>
				<form class="frm-add-qty" id="<?php echo $item['row']['activeTransactionBreakdown_id'];?>">
					<input type="number" min="0" onchange="if(this.value < 0){this.value = 0;}" class="add-qty required" id="add-qty" style="width:30px;" />
				</form>
			<?php }else{?>
		...
		<div class="div-update-qty" hidden>
			<form class="frm-update-qty" id="<?php echo $item['row']['activeTransactionBreakdown_id'];?>">
				<input type="number" min="0" onchange="if(this.value < 0){this.value = 0;}" class="update-qty required" id="update-qty" value="<?php echo $item['row']['noOfItem'];?>" class="qty" style="width:30px;" />
			</form>
		</div>
	</td>
	...
	<td>
		<div class="div-add-display-item-price">
			<?php if($item['row']['itemPrice'] == 0){?>
				<form class="frm-add-item-price" id="<?php echo $item['row']['activeTransactionBreakdown_id'];?>"> 
					...
					<?php 
						$getCustomerItemPrice = query("SELECT * FROM tbl_itempricepercustomer","WHERE itemId = :id AND customer_id = :customer_id",[":id" => $item['row']['item_id'],":customer_id" => $_SESSION["customer_id"]],"variable",1);
						
						if(!empty($getCustomerItemPrice)){
							$srp = number_format($getCustomerItemPrice['row']['price'],2,'.','');
						}else{
							$getItemSRP = query("SELECT * FROM tbl_items","WHERE itemId = :id",[":id" => $item['row']['item_id']],"variable",1);
							$srp = number_format($getItemSRP['row']['suggestedRetailPrice'],2,'.','');
						}
					?>
					<input type="number" min="0" step="0.01" onchange="if(this.value < 0){this.value = 0;}" class="add-item-price required" id="add-item-price" value="<?php echo $srp; ?>" style="width:80px;" />	
				</form>
			<?php }else{?>
		...
		<div class="div-update-item-price" hidden>
			<form class="frm-update-item-price" id="<?php echo $item['row']['activeTransactionBreakdown_id'];?>">
				<input type="number" min="0" step="0.01" onchange="if(this.value < 0){this.value = 0;}" class="update-item-price required" id="update-item-price" value="<?php echo number_format($item['row']['itemPrice'],2,'.','')?>" style="width:70px;"/>
			</form>
		</div>
	</td>
	<td >
